...
export function VideoNode({ id, data, selected }: NodeProps<VideoFlowNode>) {
  const updateNodeData = useWorkflowStore(
    (s: WorkflowState) => s.updateNodeData,
  );
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validExtensions = [".mp4", ".mov", ".webm", ".m4v", ".mpeg", ".mpg"];
    const ext = file.name.slice(file.name.lastIndexOf(".")).toLowerCase();
    const validType =
      validMimeTypes.includes(file.type) ||
      file.type === "" ||
      validExtensions.some((e) => ext === e);
    if (!validType) {
      console.error("Invalid video type:", file.type, "name:", file.name);
      return;
    }
