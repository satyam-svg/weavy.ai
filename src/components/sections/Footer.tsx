...
const Footer = () => {
  ...
  return (
    <footer ref={footerRef} className="relative bg-[#252525] overflow-hidden">
      ...
      <div className="relative bg-[#A8B1A5] max-w-350 rounded-tr-2xl md:rounded-tr-[60px] mt-8 md:mt-24 mr-0 md:mr-16 pt-12 md:pt-24 pb-8 md:pb-12 px-4 md:px-[5%]">
        <div className="max-w-350 mx-auto relative z-10">
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 mb-8 md:mb-12">
            <div className="flex flex-col md:flex-row md:max-w-[80%] gap-4 md:gap-10">
              <img
                src={FOOTER_ASSETS.logo}
                alt="Weavy Artistic Intelligence"
                className="h-10 w-auto mb-2 md:mb-6 hidden md:block"
                decoding="async"
              />
              <p className="text-white text-[13px] leading-[1.6] md:leading-normal">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-10">
              {FOOTER_LINKS.map((column) => (
                <div key={column.title} className="flex flex-col">
                  <span className="text-white/60 text-[11px] uppercase tracking-widest mb-3 font-medium">
                    {column.title}
                  </span>
          ...
          {/* Copyright */}
          <div className="font-mono text-[10px] text-[#1A1A1A]/60 uppercase tracking-widest flex gap-4">
      ...
      <Link
        href="/signin"
        className="hidden md:flex bg-[#f7ff9e] text-black absolute bottom-0 right-0 pb-10 pt-2 px-8 ml-16 pr-10 items-center justify-center rounded-tl-2xl transition-transform hover:scale-[1.02] active:scale-95 z-20"
      >
...
const HeroStatement = () => (
  <div className="flex flex-col items-start gap-2 md:gap-0 md:flex-row md:items-center mb-12 md:mb-32">
    ...
    <span className="flex items-center justify-center text-white py-2 md:py-0 md:px-8 group cursor-pointer">
      <GoPlus
        size={60}
        className="md:w-25 md:h-25 transition-all duration-300 ease-out group-hover:rotate-45 group-hover:text-[#f7ff9e]"
      />
    </span>
    <h2 className="text-white text-[clamp(2.5rem,10vw,5.5rem)] font-medium leading-[0.95] tracking-[-0.03em]">
      Human<br />Creativity
